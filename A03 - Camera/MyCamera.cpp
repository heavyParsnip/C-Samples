...
{
	// Calculate forward vector (and multiply by the distance)
	vector3 v3Forward = glm::normalize(vector3(m_v3Target - m_v3Position)) * a_fDistance;
...
void MyCamera::MoveSideways(float a_fDistance)
{
	...
	vector3 v3Forward = glm::normalize(vector3(m_v3Target - m_v3Position));
	// Calculate the right vector (and multiply by the distance)
	vector3 v3Right = glm::normalize(vector3(v3Forward.z, 0.0f, -v3Forward.x)) * a_fDistance;
...
void MyCamera::RotateCamera(float a_angleX, float a_angleY)
{
	// gimbal lock big stinky

	// Calculate forward vector
	vector3 v3Forward = glm::normalize(vector3(m_v3Target - m_v3Position));
	// Calculate the right vector
	vector3 v3Right = glm::normalize(vector3(v3Forward.z, 0.0f, -v3Forward.x));
	// Calculate the up vector
	vector3 v3Up = glm::normalize(vector3(m_v3Above - m_v3Position));

	// Calculate the rotation values, then normalize
	quaternion rotateX = glm::angleAxis(glm::radians(a_angleX), v3Right);
	quaternion rotateY = glm::angleAxis(glm::radians(a_angleY), v3Up);
	rotateX = glm::normalize(rotateX);
	rotateY = glm::normalize(rotateY);

	// Apply rotation
	m_v3Target = m_v3Position + (v3Forward * rotateX * rotateY);
	m_v3Above = m_v3Position + (v3Up * rotateY);
}